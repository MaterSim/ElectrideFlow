#!/bin/bash
#SBATCH --job-name=gen_ter_ele_csp
...
cd $HOME/SOFT/mattergen_test/ternary_electride
...
echo "=========================================="
echo "Ternary Electride Generation (CSP Mode)"
echo "=========================================="
...
if [ ! -f "$COMPOSITIONS_FILE" ]; then
    ...
    echo ""
    echo "Please run search_ternary_electrides.py first to generate compositions"
    exit 1
fi
...
if [ $SUMMARY_STATUS -ne 0 ]; then
    ...
    # Don't exit, summary is not critical
fi
...
echo ""
echo "=========================================="
echo "Ternary electride generation completed!"
echo "=========================================="
echo ""
